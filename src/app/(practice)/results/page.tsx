...
export default function ResultsPage() {
  ...
  return <main><section className="panel"><p className="eyebrow">ClientTalk Coach</p><h1>練習結果</h1><p>緊張度: {review.tensionBefore} → {review.tensionAfter}</p><p>自信度: {review.confidenceBefore} → {review.confidenceAfter}</p><section className="feedback-summary" aria-labelledby="strengths-title"><h2 id="strengths-title">今回できたこと</h2>{feedback.strengths.length ? <ul>{feedback.strengths.map((strength) => <li key={strength.id}>{strength.description}</li>)}</ul> : <p>会話を最後まで進められました。</p>}<div className="primary-feedback" data-testid="primary-feedback"><h2>次に練習すること</h2><p>{feedback.primaryImprovement.description}</p><h3>再練習</h3><p>{feedback.primaryImprovement.retryTask}</p><Link className="primary-action" href="/review">録画を振り返る</Link><Link className="secondary-link" href="/history">練習履歴を見る</Link></div></section><section className="evaluation-summary"><h2>取得できた事項</h2><ResultList facts={evaluation.capturedFacts} empty="まだありません" /><h2>未確認の重要事項</h2><ResultList facts={evaluation.missingCriticalFacts} empty="ありません" /><h2>未確認事項</h2><ResultList facts={evaluation.missingNormalFacts} empty="ありません" /></section></section></main>;
}
