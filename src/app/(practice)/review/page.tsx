...
import { loadConversationFeedback } from "@/modules/conversation-analysis/infrastructure/session-storage-feedback";
...
export default function ReviewPage() {
  ...
  const startRetry = () => {
    const feedback = loadConversationFeedback(session.id);
    ...
    const retry = startPartialRetry({ id: crypto.randomUUID(), originalSessionId: session.id, category, durationSeconds: retryDurationSeconds(60), task, createdAt: new Date().toISOString() });
    savePartialRetry(retry); setActiveRetry(retry); setRetries((current) => [...current, retry]);
  };
  const finishRetry = () => { if (!activeRetry) return; const completed = completePartialRetry(activeRetry, new Date().toISOString()); savePartialRetry(completed); setRetries((current) => current.map((item) => item.id === completed.id ? completed : item)); setActiveRetry(null); };
  const feedback = loadConversationFeedback(session.id);
  const retryTask = feedback?.primaryImprovement.retryTask ?? retryTaskFor("next-practice");
  return <main><section className="panel" aria-labelledby="review-title"><p className="eyebrow">ClientTalk Coach</p><h1 id="review-title">録画を振り返る</h1><p>動画を見なくても、マーカーと再練習だけで振り返れます。</p>{videoUrl ? <video ref={videoRef} controls src={videoUrl} aria-label="練習録画" /> : <p role="status">録画は保存されていないか、自動削除されました。分析結果は引き続き確認できます。</p>}<h2>マーカー</h2>{markers.length ? <ul>{markers.map((marker) => <li key={`${marker.category}-${marker.timestampMs}`}><button type="button" onClick={() => { if (videoRef.current) videoRef.current.currentTime = marker.timestampMs / 1_000; }}>{Math.round(marker.timestampMs / 1_000)}秒: {marker.detail}</button></li>)}</ul> : <p>表示できる音声マーカーはまだありません。</p>}<section className="primary-feedback"><h2>短い再練習</h2><p>{activeRetry?.task ?? retryTask}</p>{activeRetry ? <button className="primary-action" type="button" onClick={finishRetry}>再練習を完了する</button> : <button className="primary-action" type="button" onClick={startRetry}>60秒の再練習を開始する</button>}{retries.some((retry) => retry.status === "completed") ? <p role="status">部分再練習を完了しました。元の練習に紐付けて保存しました。</p> : null}</section><Link className="secondary-link" href="/results">結果へ戻る</Link></section></main>;
}
